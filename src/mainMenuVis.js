...
class Particle {
    ...
    randomize() {
        ...
        this.hue = this.hueSemen > .5 ? opt.h1 : opt.h2
...
--------------------*/
function setup() {
    let myVisBack = createCanvas(windowWidth, windowHeight);
    myVisBack.parent('play_area');
...
Resize
--------------------*/
let visualiser_canvas_timer;
function windowResized() {
    clearTimeout(visualiser_canvas_timer);
    resizeCanvas(windowWidth, windowHeight)
    visualiser_canvas_timer = setTimeout(function () {
        Particles = [];
        for (let i = 0; i < opt.particles; i++) {
            Particles.push(new Particle(Math.random() * width, Math.random() * height))
        }
